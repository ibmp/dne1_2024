...
while (<>) 
{
  ...
  my $input = $_;
  $input=~s/(\s\s\s+)/ * * /g; # replace multiple spaces with a single space
  my ($chr,$loc,$ref,@line) = split (/\s+/,$input); # split the input into chr, loc, ref, and the rest of the line
  
  my $str="$chr\t$loc"."\t".$ref; # create the string to be printed
  ...
  for (my $i = 0; $i < @line; $i += 3) {
    ...
    my $q_scores = $line[$i+2];
          ...
          {
             #print "is.zero\n";
             	$str=$str."\t0\t0\t0\t0\t0\t0\t0\t0"; # print the string with the appropriate number of tabs
	      	next;
          }
    ...
    $base_line=~s/\^.//g; # remove the ^ character from the base line
    $base_line=~s/\$//g; # remove the $ character from the base line
  ...
  ########################################
   ...
   my %nts = ( A => 0,
    ...
    g => 0,);
     ...
     my @base=split(//,$base_line); # split the base line into individual bases
     my @scores=split(//,$line[$i+2]); # split the quality scores line into individual scores
